# Selenium JavaScript + TestivAI

Visual regression testing with Selenium WebDriver and TestivAI CDP SDK.
...
The TestivAI CDP SDK injects a `window.testivaiWitness()` function into Chrome via the remote debugging protocol (port 9222). The `witness()` helper in `testivai-witness.js` wraps this call in a Selenium-friendly format.

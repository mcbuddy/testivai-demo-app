...
describe('Component Showcase - WebdriverIO', () => {
  ...
  it('should display the component showcase page', async () => {
    ...
    await expect(headerP).toHaveText('A collection of reusable React components');
  });
  ...
  it('should display alert components with correct variants', async () => {
    ...
    await expect(errorAlert).toHaveText('There was an error processing your request');
  });
  ...
  it('should display card components', async () => {
    ...
    await expect(cardText).toHaveText('Discover breathtaking views');
  ...
  it('visual regression - full page screenshot', async () => {
    ...
    await browser.executeScript('return window.testivaiWitness(arguments[0])', ['component-showcase-full-page']);
  });

  it('visual regression - header section', async () => {
    await browser.executeScript('return window.testivaiWitness(arguments[0])', ['component-showcase-header']);
  });

  it('visual regression - alert components', async () => {
    await browser.executeScript('return window.testivaiWitness(arguments[0])', ['component-showcase-alerts']);
  });

  it('visual regression - button components', async () => {
    await browser.executeScript('return window.testivaiWitness(arguments[0])', ['component-showcase-buttons']);
  });

  it('visual regression - card components', async () => {
    await browser.pause(1000); // Wait for images to load
    await browser.executeScript('return window.testivaiWitness(arguments[0])', ['component-showcase-cards']);
  });
  ...
  it('responsive design - mobile viewport', async () => {
    ...
    await browser.executeScript('return window.testivaiWitness(arguments[0])', ['component-showcase-mobile']);
  });
  ...
  it('responsive design - tablet viewport', async () => {
    ...
    await browser.executeScript('return window.testivaiWitness(arguments[0])', ['component-showcase-tablet']);
  });
});

...
class TestRunner extends EventEmitter {
  ...
  async runTests() {
    return new Promise((resolve, reject) => {
      ...
      let apiKey = process.env.TESTIVAI_API_KEY;
      if (!apiKey) {
        try {
          const envContent = fs.readFileSync(path.resolve(__dirname, '.env'), 'utf8');
          const match = envContent.match(/TESTIVAI_API_KEY=(.+)/);
          if (match) {
            apiKey = match[1].trim();
          }
        } catch (e) {
      ...
      if (!apiKey) {
        reject(new Error('TESTIVAI_API_KEY not set'));
        return;
      }
      ...
      const env = {
        ...
        TESTIVAI_API_KEY: apiKey
      };
      
      console.log('Using API key:', apiKey.substring(0, 10) + '...');

      // Run the test command
      const testCommand = `testivai run "npx wdio run wdio.conf.cjs --spec component-showcase.test.cjs"`;
  ...
  async run() {
    try {
      ...
      // Run tests (let WebdriverIO handle Chrome)
      await this.runTests();
